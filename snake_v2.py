...
wall_bg = pygame.image.load(PATH+"/wall.png")
wall_bg = pygame.transform.scale(wall_bg,(SIZE,SIZE))
playing=True
paused=True
...
class Apple():
    def __init__(self, size, snake, play_area, wall):
        self.play_area = play_area
        self.snake = snake
        self.wall = wall
        self.size = size
...
class Wall():
    def __init__(self, size, snake, play_area):
        ...
        self.wall = []
        self.levels = Levels()
        self.level = self.levels.levels[9] #switch levels
    
    def draw(self):
        for i,x in enumerate(self.level):
            for j,y in enumerate(x):
                if y:
                    self.wall.append(pygame.Rect(j*self.size,i*self.size,self.size,self.size))
                    screen.blit(wall_bg,[j*self.size,i*self.size])

class Levels():
    def __init__(self):
        self.levels = []
        for i in range(10):
            with open(PATH+f"/levels/level{i}.txt") as f:
                data = f.read()
            matrix = []
            for x in data.split('\n'):
                matrix.append([])
                for y in x.split(','):
                    matrix[-1].append(int(y))
...
class Game():
    def __init__(self, panel):
        ...
        self.wall = Wall(SIZE, self.snake.snake, self.play_area)
        self.apple = Apple(SIZE, self.snake.snake, self.play_area, self.wall.wall)
    ...
    def check_collision(self):
        global playing
        for wall in self.wall.wall:
            if wall.collidepoint(self.snake.snake[1]):
                mixer.music.load(PATH+random.choice(("/over1.mp3","/over2.mp3")))
                mixer.music.play()
                playing=False
    ...
    def run(self):
        ...
        while True:
            ...
            time.sleep(1/(SIZE*FACTOR)**(1/2.8)) #change the snake speed
